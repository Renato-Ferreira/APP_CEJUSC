...
const loginRoute = (app) => {
    ...
    .post( async (req, res) => {
        ...
        async function validaUser(user, senha) {
            ...
            iniciandoBD()
                ...
                .then( (resultado) => {
                        if (resultado == true) {
                            res.send(`<script>window.location.assign("http://192.168.0.102:3000/principal/${req.body.cpfUsuario}.${userLogado.token}.presenca");</script>`);
                        }
                        else {
                            res.send('<script>window.location.assign("http://192.168.0.102:3000");alert("Usuário ou Senha inválidos!");</script>');
                        }
                    },
                    (error) => {
    ...
    .get( (req, res ) => {
        ...
        res.send('<script>window.location.assign("http://192.168.0.102:3000");</script>');
    });
