...
const loginRoute = (app) => {
    ...
    .post( async (req, res) => {
        ...
        async function validaUser(user, senha) {
            ...
            iniciandoBD()
                ...
                .then( (resultado) => {
                        if (resultado == true) {
                            res.send(`<script>window.location.assign("http://localhost:3000/principal/${req.body.cpfUsuario}.${userLogado.token}.home");</script>`);
                        }
                        else {
                            res.send('<script>window.location.assign("http://localhost:3000");alert("Usuário ou Senha inválidos!");</script>');
                        }
                    },
                    (error) => {
    ...
    .get( (req, res ) => {
        ...
        res.send('<script>window.location.assign("http://localhost:3000");</script>');
    });
